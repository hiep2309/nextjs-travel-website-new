...
function postsToGuides(posts: TravelPost[]): GuideEntry[] {
  return posts.map((p) => ({
    id: `fs-${p.id}`,
    source: "firestore",
    title: p.title || p.name || "Bài chia sẻ",
    excerpt:
      (p.description || "").trim().slice(0, 160) +
      ((p.description || "").length > 160 ? "…" : ""),
    category: "dest",
    readMinutes: Math.max(3, Math.min(20, Math.round(((p.description || "").length || 200) / 400))),
...
export default function GuidesPage() {
  ...
  const merged = useMemo(() => [...postsToGuides(remote), ...MOCK_GUIDES], [remote]);

  const filtered = useMemo(() => {
    if (chip === "all") return merged;
    return merged.filter((g) => {
      if (g.source === "firestore") return chip === "dest";
    ...
    <div className="min-h-screen bg-[#0b1121] pb-16 pt-28 text-white">
      <div className="mx-auto max-w-[960px] px-4 sm:px-6 lg:px-8">
        ...
        <header className="mx-auto mt-8 max-w-2xl text-center">
          <h1 className="text-3xl font-bold sm:text-4xl">Cẩm nang du lịch</h1>
          <p className="mt-3 text-sm text-[#9aa5bc] sm:text-base">
            Bài từ cộng đồng (đã duyệt) và bài mẫu. Bấm để đọc chi tiết.
          </p>
        </header>
        ...
        <div className="mt-10 space-y-5">
          {filtered.map((g) => {
            const catLabel = g.source === "firestore" ? "Cộng đồng" : labelForCategory(g.category);
            return (
              <article
                key={g.id}
              ...
              >
                <Link href={g.href} className="flex flex-col sm:flex-row">
                  <span className="relative aspect-[16/10] shrink-0 sm:aspect-auto sm:h-48 sm:w-64">
